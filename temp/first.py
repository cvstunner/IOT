...
# Scolling Animations
def scroll_out_screen(speed, direction):
  for i in range (-int((oled_width+1)/speed), 0):
    for j in range (-int(oled_height), 0):
      display.pixel(i, j, 0)
    display.scroll(direction*speed,0)
    display.show()
...
def sync():
    ...
    if status == 1:  
        ...
        print()
        print(rtc.datetime())
        task = asyncio.create_task(displayTime())
    else:
        return
...
def toggle():
    print('Btn Clicked!')
    scroll_out_screen(16, -1)
    display.fill(0)
    write20.text('ECG', 38, 25)
    display.show()
    task.cancel()
...
def toggle_two():
    ...
    scroll_out_screen(16, 1)
    asyncio.create_task(displayTime())
